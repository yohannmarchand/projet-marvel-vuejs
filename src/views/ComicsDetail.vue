...
export default {
  ...
  computed: {
    comicThumbnail() {
      return this.comic.id ? this.comic.images.length ? `${this.comic.images[0].path}.${this.comic.images[0].extension}` : `${this.comic.thumbnail.path}.${this.comic.thumbnail.extension}` : '';
    },
  },
  ...
  created() {
    ...
    .then(() => {
      $api.get(`/comics/${this.$route.params.id}/characters`).then(({data}) => {
